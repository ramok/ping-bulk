...
- **Running Tests**: Navigate to the root directory and execute `pytest tests/` to run the test suite. Tests can also be run in parallel using `pytest-xdist` (`pytest -n auto`) as test sessions are isolated using unique `tmux` session names. Ensure any new features include appropriate tests, especially for complex parsing (like brace expansion or SSH directives). For more details on the testing infrastructure and how to write tests using the headless tmux environment, refer to `tests/AI_TESTING_GUIDE.md` and `SKILL.md`.
